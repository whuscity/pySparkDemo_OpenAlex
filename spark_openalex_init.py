...
class OpenAlex():
  ...
  streams = {
    'Authors' : ('authors.tsv', ['id:ulong', 'orcid:string?', 'display_name:string?', 'display_name_alternatives:string?', 'works_count:int?', 'cited_by_count:int?', 'last_known_institution:string?', 'works_api_url:string', 'updated_date:DateTime?']),
    'AuthorsIds' : ('authors_ids.tsv', ['author_id:ulong', 'openalex:string?', 'orcid:string?', 'scopus:string?', 'twitter:string?', 'wikipedia:string?', 'mag:int?']),
    'AuthorsCountsByYear' : ('authors_counts_by_year.tsv', ['author_id:ulong', 'year:int', 'works_count:int?', 'cited_by_count:int?']),
    'Concepts' : ('concepts.tsv', ['id:ulong', 'wikidata:string?', 'display_name:string?', 'level:int?', 'description:string?', 'works_count:int?', 'cited_by_count:int?', 'image_url:string?', 'image_thumbnail_url:string?', 'works_api_url:string', 'updated_date:DateTime?']),
    'ConceptsAncestors' : ('concepts_ancestors.tsv', ['concept_id:ulong', 'ancestor_id:string']),
    'ConceptsCountsByYear' : ('concepts_counts_by_year.tsv', ['concept_id:ulong', 'year:int', 'works_count:int?', 'cited_by_count:int?']),
    'ConceptsIds' : ('concepts_ids.tsv', ['concept_id:ulong', 'openalex:string?', 'wikidata:string?', 'wikipedia:string?', 'umls_aui:string?', 'umls_cui:string', 'mag:int?']),
    'ConceptsRelatedConcepts' : ('concepts_related_concepts.tsv' , ['concept_id:ulong', 'related_concept_id:ulong', 'score:float?']),
    'Institutions' : ('institutions.tsv', ['id:ulong', 'ror:string?', 'display_name:string?', 'country_code:string?', 'type:string?', 'homepage_url:string?', 'image_url:string?', 'image_thumbnail_url:string?', 'display_name_acronyms:string?', 'display_name_alternatives:string?', 'works_count:int?', 'cited_by_count:int?', 'works_api_url:string', 'updated_date:DateTime?']),
    'InstitutionsAssociatedInstitutions' : ('institutions_associated_institutions.tsv', ['institution_id:ulong', 'associated_institution_id:ulong', 'relationship:string?']),
    ...
    'WorksConcepts' : ('works_concepts.tsv', ['work_id:ulong', 'concept_id:ulong', 'score:float?']),
    'WorksHostVenues' : ('works_host_venues.tsv', ['work_id:ulong', 'venue_id:ulong', 'url:string?', 'is_oa:bool?', 'version:string?', 'licence:string?']),
    'WorksIds' : ('works_ids.tsv', ['work_id:ulong', 'openalex:string?', 'doi:string?', 'mag:int?', 'pmid:string?']),
    'WorksMesh' : ('works_mesh.tsv', ['work_id:ulong', 'descriptor_ui:string?', 'descriptor_name:string?', 'qualifier_ui:string?', 'qualifier_name:string?']),
    'WorksOpenAccess' : ('works_open_access.tsv', ['work_id:ulong', 'is_oa:bool?', 'oa_status:string?', 'oa_url:string?']),
    'WorksRelatedWorks': ('works_related_works.tsv', ['work_id:ulong', 'related_work_id:ulong']),
    'WorksReferencedWorks' : ('works_referenced_works.tsv', ['work_id:ulong', 'referenced_work_id:ulong']),
    'Venues' : ('venues.tsv', ['id:ulong', 'issn_l:string?', 'issn:string?', 'display_name:string?', 'publisher:string?', 'works_count:int?', 'cited_by_count:int?', 'is_oa:bool?', 'is_in_doaj:bool?', 'homepage_url:string?', 'works_api_url:string?', 'updated_date:DateTime?']),
    'VenuesCountsByYear' : ('venues_counts_by_year.tsv', ['venue_id:ulong', 'year:int', 'works_count:int?', 'cited_by_count:int?']),
    'VenuesIds' : ('venues_ids.tsv', ['venue_id:ulong', 'openalex:string?', 'issn_l:string?', 'issn:string?', 'mag:int?'])
  }
